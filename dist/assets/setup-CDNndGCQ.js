...
import{i as j,a as o,j as M,b as G,t as m,_ as A,s as k,k as N}from"./index-CxJ7gP_v.js";import{showUpgradeModal as P}from"./modal-DK6Az47R.js";import{d as O}from"./error-diagnosis-BawKPn5g.js";async function J(){const t=document.createElement("div");return t.className="page",t.innerHTML=`
  ...
  `,t.querySelector("#btn-recheck").addEventListener("click",()=>S(t)),S(t),t}async function S(t){var z,$;const c=t.querySelector("#setup-steps");c.innerHTML=`
    ...
    <div class="stat-card loading-placeholder" style="height:48px;margin-top:8px"></div>
    <div class="stat-card loading-placeholder" style="height:48px;margin-top:8px"></div>
  `,j("get_version_info","check_node","check_git","get_services_status","check_installation");const[p,r,n,l,v]=await Promise.allSettled([o.checkNode(),o.checkGit(),o.getServicesStatus(),o.checkInstallation(),o.getVersionInfo()]),g=p.status==="fulfilled"?p.value:{installed:!1},h=r.status==="fulfilled"?r.value:{installed:!1},b=n.status==="fulfilled"&&((z=n.value)==null?void 0:z.length)>0&&(($=n.value[0])==null?void 0:$.cli_installed)!==!1;let y=l.status==="fulfilled"?l.value:{installed:!1};const C=v.status==="fulfilled"?v.value:null;if(b&&!y.installed)try{const a=await o.initOpenclawConfig();a!=null&&a.created&&(y=await o.checkInstallation())}catch(a){console.warn("[setup] 自动初始化配置失败:",a)}h.installed&&o.configureGitHttps().catch(()=>{}),R(t,{node:g,git:h,cliOk:b,config:y,version:C})}function E(t){return`<span style="color:${t?"var(--success)":"var(--text-tertiary)"};font-weight:700;width:18px;display:inline-block">${t?"✓":"✗"}</span>`}function R(t,{node:c,git:p,cliOk:r,config:n,version:l}){const v=t.querySelector("#setup-steps"),g=c.installed,h=(p==null?void 0:p.installed)||!1,b=g&&r&&n.installed;let y="";y+=`
    <div class="config-section" style="text-align:left;max-width:720px;margin-left:auto;margin-right:auto">
      ...
      ${g?`<p style="color:var(--success);font-size:var(--font-size-sm)">已安装 ${c.version||""}</p>`:`<p style="color:var(--text-secondary);font-size:var(--font-size-sm);margin-bottom:var(--space-sm)">
          ...
          <div style="margin-top:var(--space-sm);padding:8px 12px;background:var(--bg-tertiary);border-radius:var(--radius-sm);font-size:var(--font-size-xs);color:var(--text-secondary);line-height:1.6">
            ...
            ${M()?`macOS 上从 Finder 启动可能找不到 Node.js。试试关掉 ClawStar 后从终端启动：<br>
            ...
            <div style="margin-top:6px;display:flex;gap:6px">
              <input id="input-node-path" type="text" placeholder="${M()?"/usr/local/bin":"F:\\\\AI\\\\Node"}"
  ...
  `,y+=`
    <div class="config-section" style="text-align:left;max-width:720px;margin-left:auto;margin-right:auto;${g?"":"opacity:0.4;pointer-events:none"}">
      ...
      ${h?`<p style="color:var(--success);font-size:var(--font-size-sm)">已安装 ${p.version||""}</p>
...
请帮我分析问题并给出解决步骤。如果需要，请使用工具帮我检查系统环境。`}function U(t,c,p){var n,l,v,g,h,b,y,C,z,$;(n=t.querySelector("#btn-goto-assistant"))==null||n.addEventListener("click",()=>{window.location.hash="/assistant"}),(l=t.querySelector("#btn-ask-ai-help"))==null||l.addEventListener("click",()=>{if(p){const a=B(p);sessionStorage.setItem("assistant-auto-prompt",a)}window.location.hash="/assistant"}),(v=t.querySelector("#btn-enter"))==null||v.addEventListener("click",()=>{window.location.hash="/dashboard"}),(g=t.querySelector("#btn-goto-models"))==null||g.addEventListener("click",()=>{window.location.hash="/models"}),(h=t.querySelector("#btn-goto-gateway"))==null||h.addEventListener("click",()=>{window.location.hash="/gateway"}),(b=t.querySelector("#btn-goto-channels"))==null||b.addEventListener("click",()=>{window.location.hash="/channels"}),(y=t.querySelector("#btn-auto-install-git"))==null||y.addEventListener("click",async()=>{const a=t.querySelector("#btn-auto-install-git"),s=t.querySelector("#git-install-result");a.disabled=!0,a.textContent="安装中...",s&&(s.style.display="block",s.innerHTML='<span style="color:var(--text-tertiary)">正在安装 Git，请稍候...</span>');try{const e=await o.autoInstallGit();s&&(s.innerHTML=`<span style="color:var(--success)">✓ ${e}</span>`),m("Git 安装成功","success"),o.configureGitHttps().catch(()=>{}),setTimeout(()=>S(t),1e3)}catch(e){const d=String(e.message||e);s&&(s.innerHTML=`<div>
        ...
        </div>`),m("Git 自动安装失败，请手动安装","warning")}finally{a.disabled=!1,a.textContent="一键安装 Git"}}),(C=t.querySelector("#btn-init-config"))==null||C.addEventListener("click",async()=>{const a=t.querySelector("#btn-init-config");a.disabled=!0,a.textContent="初始化中...";try{const s=await o.initOpenclawConfig();s!=null&&s.created?m("配置文件已创建","success"):m((s==null?void 0:s.message)||"配置文件已存在","info"),setTimeout(()=>S(t),500)}catch(s){m("初始化失败: "+s,"error"),a.disabled=!1,a.textContent="一键初始化配置"}}),(z=t.querySelector("#btn-scan-node"))==null||z.addEventListener("click",async()=>{const a=t.querySelector("#btn-scan-node"),s=t.querySelector("#scan-result");a.disabled=!0,a.textContent="扫描中...",s.style.display="block",s.innerHTML='<span style="color:var(--text-tertiary)">正在扫描常见安装路径...</span>';try{const e=await o.scanNodePaths();e.length===0?s.innerHTML='<span style="color:var(--warning)">未找到 Node.js 安装，请手动指定路径或下载安装。</span>':(s.innerHTML=e.map(d=>`<div style="display:flex;align-items:center;gap:6px;margin-top:4px">
          ...
          </div>`).join(""),s.querySelectorAll(".btn-use-path").forEach(d=>{d.addEventListener("click",async()=>{await o.saveCustomNodePath(d.dataset.path),m("Node.js 路径已保存，正在重新检测...","success"),setTimeout(()=>S(t),300)})}))}catch(e){s.innerHTML=`<span style="color:var(--danger)">扫描失败: ${e}</span>`}finally{a.disabled=!1,a.innerHTML=`${G("search",12)} 自动扫描`}}),($=t.querySelector("#btn-check-path"))==null||$.addEventListener("click",async()=>{var d;const a=t.querySelector("#input-node-path"),s=t.querySelector("#scan-result"),e=(d=a==null?void 0:a.value)==null?void 0:d.trim();if(!e){m("请输入 Node.js 安装目录","warning");return}s.style.display="block",s.innerHTML='<span style="color:var(--text-tertiary)">检测中...</span>';try{const x=await o.checkNodeAtPath(e);x.installed?(await o.saveCustomNodePath(e),s.innerHTML=`<span style="color:var(--success)">✓ 找到 Node.js ${x.version}，路径已保存</span>`,m("Node.js 路径已保存，正在重新检测...","success"),setTimeout(()=>S(t),300)):s.innerHTML='<span style="color:var(--warning)">该目录下未找到 node 可执行文件，请确认路径正确。</span>'}catch(x){s.innerHTML=`<span style="color:var(--danger)">检测失败: ${x}</span>`}});const r=t.querySelector("#btn-install");!r||!c||r.addEventListener("click",async()=>{var q,I;const a=((q=t.querySelector('input[name="install-source"]:checked'))==null?void 0:q.value)||"chinese",s=(I=t.querySelector("#registry-select"))==null?void 0:I.value,e=P("安装 OpenClaw");let d,x;N(!0);const _=()=>{N(!1),d==null||d(),x==null||x(),T==null||T(),H==null||H()};let T,H;try{if(window.__TAURI_INTERNALS__){const{listen:f}=await A(async()=>{const{listen:u}=await import("./event-DjZVAIBO.js");return{listen:u}},__vite__mapDeps([0,1]));if(d=await f("upgrade-log",u=>e.appendLog(u.payload)),x=await f("upgrade-progress",u=>e.setProgress(u.payload)),T=await f("upgrade-done",async u=>{_(),e.setDone(typeof u.payload=="string"?u.payload:"安装完成"),e.appendLog("正在安装 Gateway 服务...");try{await o.installGateway(),e.appendHtmlLog(`${k("ok",14)} Gateway 服务已安装`)}catch(i){e.appendHtmlLog(`${k("warn",14)} Gateway 安装失败: ${i}`)}try{const i=await o.readOpenclawConfig();if(i){let w=!1;i.gateway||(i.gateway={}),i.gateway.mode||(i.gateway.mode="local",w=!0,e.appendHtmlLog(`${k("ok",14)} 已设置 Gateway 运行模式为 local`)),(!i.tools||i.tools.profile!=="full")&&(i.tools={profile:"full",sessions:{visibility:"all"},...i.tools||{}},i.tools.profile="full",i.tools.sessions||(i.tools.sessions={}),i.tools.sessions.visibility="all",w=!0,e.appendHtmlLog(`${k("ok",14)} 已开启 Agent 工具全部权限`)),w&&await o.writeOpenclawConfig(i)}}catch(i){e.appendHtmlLog(`${k("warn",14)} 自动配置失败: ${i}`)}m("OpenClaw 安装成功","success"),setTimeout(()=>window.location.reload(),1500)}),H=await f("upgrade-error",async u=>{_();const i=String(u.payload||"未知错误");e.appendLog(i),await new Promise(W=>setTimeout(W,150));const w=e.getLogText()+`
`+i,L=O(w);e.setError(L.title),L.hint&&e.appendLog(""),L.hint&&e.appendHtmlLog(`${k("info",14)} ${L.hint}`),L.command&&e.appendHtmlLog(`${G("clipboard",14)} ${L.command}`),window.__openAIDrawerWithError&&window.__openAIDrawerWithError({title:L.title,error:w,scene:"初始安装 OpenClaw",hint:L.hint})}),s){e.appendLog(`设置 npm 镜像源: ${s}`);try{await o.setNpmRegistry(s)}catch{}}await o.installOpenclaw(a),e.appendLog("后台安装任务已启动，请等待完成...")}else{if(e.appendLog("Web 模式：安装日志不可用，请等待完成..."),s){e.appendLog(`设置 npm 镜像源: ${s}`);try{await o.setNpmRegistry(s)}catch{}}const f=await o.installOpenclaw(a);e.setDone(f),m("OpenClaw 安装成功","success"),setTimeout(()=>window.location.reload(),1500),_()}}catch(f){_();const u=String(f);e.appendLog(u);const i=e.getLogText()+`
