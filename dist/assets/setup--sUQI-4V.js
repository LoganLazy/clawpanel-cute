...
import{i as j,a as o,j as N,b as G,t as m,_ as A,s as k,k as q}from"./index-ZS5CnNlZ.js";import{showUpgradeModal as P}from"./modal-DK6Az47R.js";import{d as O}from"./error-diagnosis-BawKPn5g.js";async function J(){const t=document.createElement("div");return t.className="page",t.innerHTML=`
  ...
  `,t.querySelector("#btn-recheck").addEventListener("click",()=>S(t)),S(t),t}async function S(t){var z,$;const c=t.querySelector("#setup-steps");c.innerHTML=`
    ...
    <div class="stat-card loading-placeholder" style="height:48px;margin-top:8px"></div>
    <div class="stat-card loading-placeholder" style="height:48px;margin-top:8px"></div>
  `,j("get_version_info","check_node","check_git","get_services_status","check_installation");const[p,r,n,l,v]=await Promise.allSettled([o.checkNode(),o.checkGit(),o.getServicesStatus(),o.checkInstallation(),o.getVersionInfo()]),g=p.status==="fulfilled"?p.value:{installed:!1},x=r.status==="fulfilled"?r.value:{installed:!1},b=n.status==="fulfilled"&&((z=n.value)==null?void 0:z.length)>0&&(($=n.value[0])==null?void 0:$.cli_installed)!==!1;let y=l.status==="fulfilled"?l.value:{installed:!1};const C=v.status==="fulfilled"?v.value:null;if(b&&!y.installed)try{const s=await o.initOpenclawConfig();s!=null&&s.created&&(y=await o.checkInstallation())}catch(s){console.warn("[setup] 自动初始化配置失败:",s)}x.installed&&o.configureGitHttps().catch(()=>{}),R(t,{node:g,git:x,cliOk:b,config:y,version:C})}function E(t){return`<span style="color:${t?"var(--success)":"var(--text-tertiary)"};font-weight:700;width:18px;display:inline-block">${t?"✓":"✗"}</span>`}function R(t,{node:c,git:p,cliOk:r,config:n,version:l}){const v=t.querySelector("#setup-steps"),g=c.installed,x=(p==null?void 0:p.installed)||!1,b=g&&r&&n.installed;let y="";y+=`
  ...
  `,y+=`
    <div class="config-section" style="text-align:left;max-width:720px;margin-left:auto;margin-right:auto;${g?"":"opacity:0.4;pointer-events:none"}">
      ...
      ${x?`<p style="color:var(--success);font-size:var(--font-size-sm)">已安装 ${p.version||""}</p>
...
请帮我分析问题并给出解决步骤。如果需要，请使用工具帮我检查系统环境。`}function U(t,c,p){var n,l,v,g,x,b,y,C,z,$;(n=t.querySelector("#btn-goto-assistant"))==null||n.addEventListener("click",()=>{window.location.hash="/assistant"}),(l=t.querySelector("#btn-ask-ai-help"))==null||l.addEventListener("click",()=>{if(p){const s=B(p);sessionStorage.setItem("assistant-auto-prompt",s)}window.location.hash="/assistant"}),(v=t.querySelector("#btn-enter"))==null||v.addEventListener("click",()=>{window.location.hash="/dashboard"}),(g=t.querySelector("#btn-goto-models"))==null||g.addEventListener("click",()=>{window.location.hash="/models"}),(x=t.querySelector("#btn-goto-gateway"))==null||x.addEventListener("click",()=>{window.location.hash="/gateway"}),(b=t.querySelector("#btn-goto-channels"))==null||b.addEventListener("click",()=>{window.location.hash="/channels"}),(y=t.querySelector("#btn-auto-install-git"))==null||y.addEventListener("click",async()=>{const s=t.querySelector("#btn-auto-install-git"),a=t.querySelector("#git-install-result");s.disabled=!0,s.textContent="安装中...",a&&(a.style.display="block",a.innerHTML='<span style="color:var(--text-tertiary)">正在安装 Git，请稍候...</span>');try{const e=await o.autoInstallGit();a&&(a.innerHTML=`<span style="color:var(--success)">✓ ${e}</span>`),m("Git 安装成功","success"),o.configureGitHttps().catch(()=>{}),setTimeout(()=>S(t),1e3)}catch(e){const d=String(e.message||e);a&&(a.innerHTML=`<div>
          ...
          </div>`).join(""),a.querySelectorAll(".btn-use-path").forEach(d=>{d.addEventListener("click",async()=>{await o.saveCustomNodePath(d.dataset.path),m("Node.js 路径已保存，正在重新检测...","success"),setTimeout(()=>S(t),300)})}))}catch(e){a.innerHTML=`<span style="color:var(--danger)">扫描失败: ${e}</span>`}finally{s.disabled=!1,s.innerHTML=`${G("search",12)} 自动扫描`}}),($=t.querySelector("#btn-check-path"))==null||$.addEventListener("click",async()=>{var d;const s=t.querySelector("#input-node-path"),a=t.querySelector("#scan-result"),e=(d=s==null?void 0:s.value)==null?void 0:d.trim();if(!e){m("请输入 Node.js 安装目录","warning");return}a.style.display="block",a.innerHTML='<span style="color:var(--text-tertiary)">检测中...</span>';try{const h=await o.checkNodeAtPath(e);h.installed?(await o.saveCustomNodePath(e),a.innerHTML=`<span style="color:var(--success)">✓ 找到 Node.js ${h.version}，路径已保存</span>`,m("Node.js 路径已保存，正在重新检测...","success"),setTimeout(()=>S(t),300)):a.innerHTML='<span style="color:var(--warning)">该目录下未找到 node 可执行文件，请确认路径正确。</span>'}catch(h){a.innerHTML=`<span style="color:var(--danger)">检测失败: ${h}</span>`}});const r=t.querySelector("#btn-install");!r||!c||r.addEventListener("click",async()=>{var I,M;const s=((I=t.querySelector('input[name="install-source"]:checked'))==null?void 0:I.value)||"official",a=(M=t.querySelector("#registry-select"))==null?void 0:M.value,e=P("安装 OpenClaw");let d,h;q(!0);const _=()=>{q(!1),d==null||d(),h==null||h(),T==null||T(),H==null||H()};let T,H;try{if(window.__TAURI_INTERNALS__){const{listen:f}=await A(async()=>{const{listen:u}=await import("./event-DjZVAIBO.js");return{listen:u}},__vite__mapDeps([0,1]));if(d=await f("upgrade-log",u=>e.appendLog(u.payload)),h=await f("upgrade-progress",u=>e.setProgress(u.payload)),T=await f("upgrade-done",async u=>{_(),e.setDone(typeof u.payload=="string"?u.payload:"安装完成"),e.appendLog("正在安装 Gateway 服务...");try{await o.installGateway(),e.appendHtmlLog(`${k("ok",14)} Gateway 服务已安装`)}catch(i){e.appendHtmlLog(`${k("warn",14)} Gateway 安装失败: ${i}`)}try{const i=await o.readOpenclawConfig();if(i){let w=!1;i.gateway||(i.gateway={}),i.gateway.mode||(i.gateway.mode="local",w=!0,e.appendHtmlLog(`${k("ok",14)} 已设置 Gateway 运行模式为 local`)),(!i.tools||i.tools.profile!=="full")&&(i.tools={profile:"full",sessions:{visibility:"all"},...i.tools||{}},i.tools.profile="full",i.tools.sessions||(i.tools.sessions={}),i.tools.sessions.visibility="all",w=!0,e.appendHtmlLog(`${k("ok",14)} 已开启 Agent 工具全部权限`)),w&&await o.writeOpenclawConfig(i)}}catch(i){e.appendHtmlLog(`${k("warn",14)} 自动配置失败: ${i}`)}m("OpenClaw 安装成功","success"),setTimeout(()=>{window.location.hash?window.location.hash="#/dashboard":window.location.href="/#/dashboard"},1200)}),H=await f("upgrade-error",async u=>{_();const i=String(u.payload||"未知错误");e.appendLog(i),await new Promise(W=>setTimeout(W,150));const w=e.getLogText()+`
`+i,L=O(w);e.setError(L.title),L.hint&&e.appendLog(""),L.hint&&e.appendHtmlLog(`${k("info",14)} ${L.hint}`),L.command&&e.appendHtmlLog(`${G("clipboard",14)} ${L.command}`),window.__openAIDrawerWithError&&window.__openAIDrawerWithError({title:L.title,error:w,scene:"初始安装 OpenClaw",hint:L.hint})}),a){e.appendLog(`设置 npm 镜像源: ${a}`);try{await o.setNpmRegistry(a)}catch{}}await o.installOpenclaw(s),e.appendLog("后台安装任务已启动，请等待完成...")}else{if(e.appendLog("Web 模式：安装日志不可用，请等待完成..."),a){e.appendLog(`设置 npm 镜像源: ${a}`);try{await o.setNpmRegistry(a)}catch{}}const f=await o.installOpenclaw(s);e.setDone(f),m("OpenClaw 安装成功","success"),setTimeout(()=>{window.location.hash?window.location.hash="#/dashboard":window.location.href="/#/dashboard"},1200),_()}}catch(f){_();const u=String(f);e.appendLog(u);const i=e.getLogText()+`
